    ...
    */
    addRole() {
      ...
      this.$prompt("请输入新名称", "添加角色", {
        ...
        .then(({ value }) => {
          this.$api.acl.role.save({ roleName: value }).then((result) => {
    ...
    */
    removeRole({ id, roleName }) {
      this.$confirm(`确定删除 '${roleName}' 吗?`, "提示", {
        ...
        .then(async () => {
          const result = await this.$api.acl.role.removeById(id);
    ...
    removeRoles() {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          ...
          const result = await this.$api.acl.role.removeRoles(ids);
          this.getRoles();
          this.$message({
            type: "success",
            message: "批量删除成功!",
